...
public class HelloController implements Initializable {
    ...
    private ListTask tasks; //рабочий список
    ...
    @FXML
    protected void onAddNewTask() {
        try{
            String name2 = name.getText();
            String time2 = time.getText();
            String status2 = status.getText();

            impl.addTask(new Task(0, name2, time2, status2));
            table.setItems(FXCollections.observableList(impl.getAllTasks()));
        }catch (Exception e){
            e.getMessage();
        }
    }
